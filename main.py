...
# Camera params support for up to 2 cameras for now
...
def estimate_pose(corners, tag_size=0.16):
...
def main():
    camera_sources = [0, 1]
    frame_grabbers = [FrameGrabber(src) for src in camera_sources]
        ...
        for fg in frame_grabbers:
            ...
            for r in results:
                if r.tag_id in validTagIds:
                    corners = np.array([r.corners], dtype=float).reshape((4, 2))
                    rvec, tvec = estimate_pose(corners)
                    ...
                    print(f"Camera {fg.cap}: Tag {r.tag_id} Pose: Rvec: {rvec}, Tvec: {tvec}")

            cv2.imshow(f'AprilTag Detection Camera {fg.cap}', frame)
